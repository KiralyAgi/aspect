  ...
  namespace Postprocess
  {
    ...
    LPO<dim>::euler_angles_from_rotation_matrix(const Tensor<2,3> &rotation_matrix) const
    {
      ...
      if (theta != 0.0 && theta != dealii::numbers::PI)
        {
          //
          phi1  = std::atan2(rotation_matrix[2][0]/sin(theta),rotation_matrix[2][1]/-sin(theta)); //Modified by Agi and Yijun
          phi2  = std::atan2(rotation_matrix[0][2]/sin(theta),rotation_matrix[1][2]/sin(theta));  //Modified by Agi and Yijun
        }
      else
        {
          ...
          if (theta == 0.0)
            {
              phi2 = - phi1 - std::atan2(-rotation_matrix[0][1],rotation_matrix[0][0]); //Modified by Agi and Yijun
            }
          else
            {
